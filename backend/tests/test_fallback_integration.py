...
class TestFallbackFlow:
    ...
    async def test_ai_failure_triggers_fallback(self):
        ...
        # 模拟 AI 服务失败
        with patch.object(monitor, 'check_ai_health', return_value=False):
            # 执行 3 次健康检查（达到失败阈值）
            for i in range(3):
    ...
    @pytest.mark.asyncio
    async def test_fallback_mode_uses_local_engine(self):
        """测试降级模式使用本地引擎"""
        print("\n=== 测试2: 降级模式使用本地引擎 ===")
        ...
        assert result.execution_time < 3.0, "执行时间应小于 3 秒"
    ...
    @pytest.mark.asyncio
    async def test_fallback_notice_in_response(self):
        """测试降级通知包含在响应中"""
        print("\n=== 测试3: 降级通知包含在响应中 ===")
...
class TestRecoveryFlow:
    ...
    @pytest.mark.asyncio
    async def test_ai_recovery_triggers_normal_mode(self):
        """测试 AI 服务恢复触发正常模式"""
        print("\n=== 测试4: AI 服务恢复触发正常模式 ===")
        ...
        # 模拟 AI 服务恢复
        with patch.object(monitor, 'check_ai_health', return_value=True):
            # 执行 2 次健康检查（达到恢复阈值）
            for i in range(2):
                await monitor._perform_health_check()
    ...
    @pytest.mark.asyncio
    async def test_fallback_duration_tracking(self):
        """测试降级持续时间跟踪"""
        print("\n=== 测试5: 降级持续时间跟踪 ===")
...
class TestConcurrentScenarios:
    ...
    @pytest.mark.asyncio
    async def test_concurrent_validations_in_fallback_mode(self):
        """测试降级模式下的并发验证"""
        print("\n=== 测试6: 降级模式下的并发验证 ===")
    ...
    async def test_concurrent_health_checks(self):
        ...
        # 验证检查次数
        assert monitor.total_checks == num_concurrent, f"应该执行 {num_concurrent} 次检查"
        
        print(f"  ✓ 并发健康检查成功")
        print(f"    - 检查次数: {monitor.total_checks}")
...
class TestConfigurationHotReload:
    ...
    @pytest.mark.asyncio
    async def test_config_reload_without_restart(self):
        """测试无需重启的配置重载"""
        print("\n=== 测试8: 配置重载（无需重启）===")
    ...
    @pytest.mark.asyncio
    async def test_invalid_config_fallback(self):
        """测试无效配置的回退机制"""
        print("\n=== 测试9: 无效配置回退机制 ===")
        
        # 创建临时配置文件
        temp_config_path = "backend/config/test_invalid_config.json"
            ...
            "rules": [
                {
                    ...
                    "critical": False,
                    "rule_type": "pattern",
                    "parameters": {"pattern": "test", "field": "content"},
        ...
        with open(temp_config_path, 'w', encoding='utf-8') as f:
            json.dump(valid_config, f, ensure_ascii=False, indent=2)
        
        # 初始化配置管理器
        config_manager = RulesConfigManager(temp_config_path)
        initial_config = await config_manager.load_config()
        
        assert initial_config is not None, "初始配置应该加载成功"
        print(f"  ✓ 有效配置加载成功: {len(initial_config.rules)} 个规则")
        ...
        # 应该回退到上一个有效配置
        assert reloaded_config is not None, "应该回退到上一个有效配置"
        assert len(reloaded_config.rules) == len(initial_config.rules), "应该使用上一个有效配置"
        
        print(f"  ✓ 无效配置回退成功，使用上一个有效配置")
        
        # 清理临时文件
        Path(temp_config_path).unlink(missing_ok=True)
    
    @pytest.mark.asyncio
    async def test_rule_toggle_without_restart(self):
        """测试动态启用/禁用规则"""
        print("\n=== 测试10: 动态启用/禁用规则 ===")
        ...
        # 验证状态已改变
        updated_rules = config_manager.get_enabled_rules()
...
class TestPerformanceMetrics:
    ...
    @pytest.mark.asyncio
    async def test_slow_rule_identification(self):
        """测试慢规则识别"""
        print("\n=== 测试12: 慢规则识别 ===")
